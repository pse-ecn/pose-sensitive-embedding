...
def resnet_v1_views(inputs,
	...
	Args:
	  ...
	  blocks: A list of length equal to the number of ResNet blocks. Each element
		is a resnet_utils_views.Block object describing the units in the block.
	...
	with tf.variable_scope(scope, 'resnet_v1', [inputs], reuse=reuse) as sc:
		...
		with slim.arg_scope([slim.conv2d, bottleneck, resnet_utils_views.stack_blocks_dense], outputs_collections=end_points_collection):
			with slim.arg_scope([slim.batch_norm, slim.dropout], is_training=is_training):
				...
				net = slim.conv2d(net, 1536, [1, 1], activation_fn=tf.nn.relu, normalizer_fn=slim.batch_norm, scope='pre_logits')
