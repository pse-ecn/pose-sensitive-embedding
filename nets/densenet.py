...
@slim.add_arg_scope
def _conv(inputs, num_filters, kernel_size, dropout_rate=None, scope=None, outputs_collections=None):
...
@slim.add_arg_scope
def _conv_block(inputs, num_filters, scope=None, outputs_collections=None, dropout_rate=None):
	with tf.variable_scope(scope, 'conv_blockx', [inputs]) as sc:
		net = inputs
		net = _conv(net, num_filters * 4, 1, scope='x1', dropout_rate=dropout_rate)
		net = _conv(net, num_filters, 3, scope='x2', dropout_rate=dropout_rate)
...
@slim.add_arg_scope
def _dense_block(inputs, num_layers, num_filters, growth_rate, grow_num_filters=True, scope=None, outputs_collections=None, dropout_rate=None):
	with tf.variable_scope(scope, 'dense_blockx', [inputs]) as sc:
		net = inputs
		for i in range(num_layers):
			branch = i + 1
			net = _conv_block(net, growth_rate, scope='conv_block' + str(branch), dropout_rate=dropout_rate)
...
@slim.add_arg_scope
def _transition_block(inputs, num_filters, compression=1.0, scope=None, outputs_collections=None, dropout_rate=None):
	num_filters = int(num_filters * compression)
	with tf.variable_scope(scope, 'transition_blockx', [inputs]) as sc:
		net = inputs
		net = _conv(net, num_filters, 1, scope='blk', dropout_rate=dropout_rate)
...
def densenet(inputs, num_classes=1000, reduction=None, growth_rate=None, num_filters=None, num_layers=None, dropout_rate=None, is_training=True, reuse=None, scope=None):
	...
	with tf.variable_scope(scope, 'densenetxxx', [inputs, num_classes], reuse=reuse) as sc:
		...
		with slim.arg_scope([slim.batch_norm, slim.dropout], is_training=is_training), slim.arg_scope([slim.conv2d, _conv, _conv_block, _dense_block, _transition_block],
			...
			for i in range(num_dense_blocks - 1):
				# dense blocks
				net, num_filters = _dense_block(net, num_layers[i], num_filters, growth_rate, scope='dense_block' + str(i + 1), dropout_rate=dropout_rate)

				# Add transition_block
				net, num_filters = _transition_block(net, num_filters, compression=compression, scope='transition_block' + str(i + 1), dropout_rate=dropout_rate)

			net, num_filters = _dense_block(net, num_layers[-1], num_filters, growth_rate, scope='dense_block' + str(num_dense_blocks), dropout_rate=dropout_rate)
			...
			with tf.variable_scope('final_block', [inputs]):
				...
				pre_logits = slim.flatten(net, 'pre_logits')
			...
			end_points['PreLogits'] = pre_logits
