...
def _conv(inputs, num_filters, kernel_size, stride=1, dropout_rate=None, scope=None, outputs_collections=None):
	with tf.variable_scope(scope, 'xx', [inputs]) as sc:
		...
		if dropout_rate:
			net = tf.nn.dropout(net, 1 - dropout_rate)
...
def add_views_branch(net, dropout_rate, end_points):
	with tf.variable_scope('3Views'):
		after_block1 = net  # 28x28x128
		...
		if dropout_rate:
			flat_views4 = tf.nn.dropout(flat_views4, keep_prob=1 - dropout_rate)

		views_logits = slim.fully_connected(flat_views4, NUM_VIEWS, activation_fn=None, scope='Views_logits')
...
def densenet_views_arg_scope(weight_decay=1e-4, batch_norm_decay=0.99, batch_norm_epsilon=1.1e-5):
